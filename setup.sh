...
REPO="https://raw.githubusercontent.com/Guesswho44/X/main/"
REPO2="https://raw.githubusercontent.com/Guesswho44/X/main/"
function CEKIP () {
MYIP=$(curl -sS ipv4.icanhazip.com)
IPVPS=$(curl -sS https://raw.githubusercontent.com/Guesswho44/izinsc/main/izin | grep $MYIP | awk '{print $4}')
if [[ $MYIP == $IPVPS ]]; then
domain
Pasang
else
...
echo "PIAN STORE" > /etc/xray/username
echo ""
clear
author=$name
echo ""
...
function key2(){
    ...
    echo -e "${green}└──────────────────────────────────────────┘${NC}"
        ...
        cd
        git clone https://github.com/Guesswho44/izinsc >/dev/null 2>&1
		cd izinvps
		sed -i "/# ADMIN/a ### ${author} ${hhari} ${MYIP} @VIP" 
        sed -i "/# SSHWS/a ### ${author} ${hhari} ${MYIP} ON SSHWS @VIP" 
        sleep 1
...
TIMES="10"
CHATID="1134227234"
KEY="8323811572:AAEazwGWD9ETm2T0fblSLEpWZ4g9MQdTXU4"
...
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://raw.githubusercontent.com/Guesswho44/izinsc/main/izin | grep $MYIP | awk '{print $3}' )
d1=$(date -d "$IZIN" +%s)
d2=$(date -d "$today" +%s)
EXP=$(( (d1 - d2) / 86400 ))
